...
from config import DB_PATH, ADMIN_IDS, CHANNEL_ID
...
async def handle_admin_delete_comment(update, context):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT comment_id, post_id, content FROM comments WHERE comment_id = ?",
            (comment_id,)
        )
...
async def handle_admin_delete_post(update, context):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        cursor = conn.cursor()
        cursor.execute(
            "SELECT post_id, content, category, channel_message_id FROM posts WHERE post_id = ?",
            (post_id,)
        )
...
async def handle_confirm_delete_post(update, context):
    ...
    try:
        ...
        with sqlite3.connect(DB_PATH) as conn:
            ...
            cursor.execute(
                "SELECT content, category, channel_message_id FROM posts WHERE post_id = ?",
                (post_id,)
            )
            post_info = cursor.fetchone()
